...
export const calculateGasViaFeeHistory = (
	...
	const adjustedBaseFee =
		(latestBaseFeePerGas * settings.baseFeePercentageMultiplier) / 100n;
	const priorityFees =
		feeHistory.reward
			?.map((reward) => reward[settings.percentile])
			.filter((fee) => fee !== undefined) ?? [];
	const medianPriorityFee = medianOf(priorityFees);
	const adjustedPriorityFee =
		(medianPriorityFee * settings.priorityFeePercentageMultiplier) / 100n;

	const suggestedMaxPriorityFeePerGas = max(
		adjustedPriorityFee,
